...
public class ShellGenreTest {
    ...
    public void addOkTest() {
        ...
        when(mockRepository.save(any())).thenReturn(new Genre(10L, ""));
       // Команда (длинная)
        assertThat((String) shell.evaluate(COMMAND_ADD)).isEqualTo("Новый ID: 10");
        // До Repository дошло заданное значение.
        verify(mockRepository).save(new Genre(null, "Детектив"));
    }
    ...
    public void updateOkTest() {
       ...
       // Команда (длинная) возвращает пользователю ОК.
        ...
        verify(mockRepository).save(new Genre(2L, "Детектив"));
    }
    ...
    public void deleteError2Test() {
        ...
        when(mockRepository.existsById(anyLong())).thenReturn(true);
        doThrow(new DataIntegrityViolationException("error")).when(mockRepository).deleteById(anyLong());
        // Команда (краткая) возвращает пользователю сообщение об ошибке.
        ...
        verify(mockRepository).deleteById(50L);
    }
